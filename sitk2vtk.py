...
def extractSurface(vtk_image, isovalue):
    ...
    return iso.GetOutput(), iso

def smoothSurface(meshing_filter, feature_angle=120, pass_band=0.001):
    smoother = vtk.vtkWindowedSincPolyDataFilter()
    smoother.SetInputConnection(meshing_filter.GetOutputPort());
    ...
    smoother.Update();
    vtk_smoothed_mesh = smoother.GetOutput()
    return vtk_smoothed_mesh
...
def itk2vtk(img, centre=False):
    """Convert an ITK image to a VTK image."""

    vtk_image = itk.vtk_image_from_image(img)
